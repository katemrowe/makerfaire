...
<?php
	 ...
	 */
	 function wdt_get_ajax_data(){
		...
		$json = $tbl->queryBasedConstruct($table_data['content'], array(),
 				array(
		...
		$json = apply_filters('wpdatatables_filter_server_side_data', $json);
                ...
                if($column->id_column){
                	 ...
                	 switch($column->column_type){
                	 	...
                	 	case 'float':
                                    ...
                                    $formdata[$column->orig_header] = filter_var( $formdata[$id_key], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION | FILTER_FLAG_ALLOW_THOUSAND );
                	 	default:
                                    $formdata[$column->orig_header] = $valueQuote.$formdata[$column->orig_header].$valueQuote;
                                    break;
                	 }
                	 
                	 // Defining the values for "none" input types
	...
	function wdt_upload_file(){
	  	require_once(WDT_ROOT_PATH.'lib/upload/UploadHandler.php');
	  	do_action('wpdatatables_before_upload_file');
	  	$uploadHandler = new WDTUploadHandler();
	  	exit();
	}
