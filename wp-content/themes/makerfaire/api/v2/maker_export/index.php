...
if ( $type == 'maker_export' ) {
	...
	foreach ( $query->posts as $post ) {
		...
		$maker['thumb_img_url'] = esc_url( wp_get_resized_remote_image_url( $maker_image, '80', '80' ) );
		$maker['large_image_url'] = esc_url( wp_get_resized_remote_image_url( $maker_image, '600', '600' ) );;
		...
		// Maker Email link
		$maker_email = get_post_meta( absint( $post->ID ), 'email', true );
		$maker['email'] = ( ! empty( $maker_email) ) ? $maker_email : null;
