...
class GFEntryList {
	...
	public static function leads_page( $form_id ) {
		...
		$page_links    = paginate_links(
			array(
				'base'      => admin_url( 'admin.php' ) . "?page=mf_entries&view=entries&id=$form_id&%_%" . $outputVar . $sort_qs . $dir_qs . $star_qs . $read_qs . $filter_qs,
				'format'    => 'paged=%#%',
		...
		<!--
			...
			<li>
				<a class="<?php echo $read !== null ? 'current' : '' ?>" href="?page=mf_entries&view=entries&id=<?php echo $form_id ?>&filter=unread"><?php _ex( 'Unread', 'Entry List', 'gravityforms' ); ?>
					<span class="count">(<span id="unread_count"><?php echo $unread_count ?></span>)</span></a> |
			</li>
			<li>
				<a class="<?php echo $star !== null ? 'current' : '' ?>" href="?page=mf_entries&view=entries&id=<?php echo $form_id ?>&filter=star"><?php _ex( 'Starred', 'Entry List', 'gravityforms' ); ?>
			...
			if ( GFCommon::spam_enabled( $form_id ) ) {
				?>
				<li>
					<a class="<?php echo $filter == 'spam' ? 'current' : '' ?>" href="?page=mf_entries&view=entries&id=<?php echo $form_id ?>&filter=spam"><?php _e( 'Spam', 'gravityforms' ); ?>
							...
							<div class="row-actions">
								...
								switch ( $filter ) {
									case 'trash' :
										...
										<span class="edit">
                                                            <a data-wp-lists='delete:gf_entry_list:lead_row_<?php echo $lead['id'] ?>::status=active&entry=<?php echo $lead['id'] ?>' title="<?php echo _e( 'Restore this entry', 'gravityforms' ) ?>" href="<?php echo wp_nonce_url( '?page=mf_entries', 'gf_delete_entry' ) ?>"><?php _e( 'Restore', 'gravityforms' ); ?></a>
									...
									case 'spam' :
										...
										<span class="unspam">
                                                            <a data-wp-lists='delete:gf_entry_list:lead_row_<?php echo $lead['id'] ?>::status=unspam&entry=<?php echo $lead['id'] ?>' title="<?php echo _e( 'Mark this entry as not spam', 'gravityforms' ) ?>" href="<?php echo wp_nonce_url( '?page=mf_entries', 'gf_delete_entry' ) ?>"><?php _e( 'Not Spam', 'gravityforms' ); ?></a>
											<?php echo GFCommon::current_user_can_any( 'gravityforms_delete_entries' ) ? '|' : '' ?>
                                                        </span>
									...
									default:
										...
										if ( GFCommon::spam_enabled( $form_id ) ) {
											?>
											<span class="spam">
                                                                <a data-wp-lists='delete:gf_entry_list:lead_row_<?php echo $lead['id'] ?>::status=spam&entry=<?php echo $lead['id'] ?>' title="<?php _e( 'Mark this entry as spam', 'gravityforms' ) ?>" href="<?php echo wp_nonce_url( '?page=mf_entries', 'gf_delete_entry' ) ?>"><?php _e( 'Spam', 'gravityforms' ); ?></a>
												<?php echo GFCommon::current_user_can_any( 'gravityforms_delete_entries' ) ? '|' : '' ?>
                                                            </span>
										...
										if ( GFCommon::current_user_can_any( 'gravityforms_delete_entries' ) ) {
											?>
											<span class="trash">
                                                                <a data-wp-lists='delete:gf_entry_list:lead_row_<?php echo $lead['id'] ?>::status=trash&entry=<?php echo $lead['id'] ?>' title="<?php _e( 'Move this entry to the trash', 'gravityforms' ) ?>" href="<?php echo wp_nonce_url( '?page=mf_entries', 'gf_delete_entry' ) ?>"><?php _e( 'Trash', 'gravityforms' ); ?></a>
                                                            </span>
										<?php
										}
										break;
                ...
                foreach($wpdb->get_results($sql) as $row){  
                    ...
                    foreach($wpdb->get_results($formSQL) as $formRow){  
                        ...
                        foreach($wpdb->get_results($statusSql) as $statusRow){
                            $nav .= '<li>';                            
                            $nav .= '<a href="'.$adminURL.'&sort=0&dir=DESC'.urlencode('filterField[]').'=303|is|'.$statusRow->value.'">'.$statusRow->value.' ('.$statusRow->count.')</a>';                          
                            $nav .= '</li>';
                        }
                        $nav .= '</ul>';                        
                        $nav .= '</li>';
