<?php
/*
 * Plugin Name: Maker Faire Form Plugin URI: http://insourcecode.com Description: Select the type of form and embed into post or page. Version: 0.0.2 Author: inSourceCode Author URI: http://insourcecode.com License: GPL2
 */
...
class MAKER_FAIRE_FORM {
	...
	/**
	 * The name of the current faire.
	 * This should be the title, not a slug. E.G. World Maker Faire New York 2014
	...
	/*
	 * Init the MakerFaire Class @access public =====================================================================
	 */
	public function __construct() {
		...
		add_shortcode ( 'mfform', array (
				&$this,
				'shortcode_handler' 
		) );
		
		add_action ( 'wp_ajax_nopriv_mfform_step', array (
				&$this,
				'ajax_handler' 
		) );
		add_action ( 'wp_ajax_mfform_step', array (
				&$this,
				'ajax_handler' 
		) );
		
		add_action ( 'wp_ajax_mf_delete_scheduled_event', array (
				&$this,
				'mf_delete_scheduled_event' 
		) );
		
		add_action ( 'wp_ajax_nopriv_mfform_getforms', array (
				&$this,
				'ajax_getforms' 
		) );
		add_action ( 'wp_ajax_mfform_getforms', array (
				&$this,
				'ajax_getforms' 
		) );
	...
	/*
	 * Callback for INIT - HOOK @access public =====================================================================
	 */
	...
	/*
	 * Callback for ADMIN INIT - HOOK @access public =====================================================================
	 */
	public function admin_init() {
		
		// Our new form export tool
		/*
		 * NOTE:DEBUG SELECT wp_posts.* FROM wp_posts WHERE 1=1 AND wp_posts.post_type = 'mf_form' AND (wp_posts.post_status = 'publish' OR wp_posts.post_status = 'future' OR wp_posts.post_status = 'draft' OR wp_posts.post_status = 'pending' OR wp_posts.post_status = 'private') ORDER BY wp_posts.menu_order ASC, wp_posts.post_title ASC
		 */
		...
		add_filter ( 'manage_mf_form_posts_columns', array (
				&$this,
				'columns' 
		) );
		add_filter ( 'manage_edit-mf_form_sortable_columns', array (
				&$this,
				'columns_sortable' 
		) );
		add_action ( 'manage_mf_form_posts_custom_column', array (
				&$this,
				'custom_columns' 
		), 10, 2 );
	...
	/*
	 * Set Columns for the MakerFaire Custom Post Type @access public @param array $cs Default columns =====================================================================
	 */
	public function columns($cs) {
		unset ( $cs ['author'], $cs ['title'], $cs ['date'], $cs ['editorial-metadata-featured'], $cs ['editorial-metadata-temporary-location'] );
	...
	/*
	 * Set Sortable Columns @access public @param array $cs Default sortable columns =====================================================================
	 */
	...
	/*
	 * Allow for sorting of custom colulmns @access public @param array $vars The variables for the orderby =====================================================================
	 */
	...
	/*
	 * Adds Custom Columns to the MakerFaire Custom Post Type @access public @param string $c Column ID @param integer $pid Post ID =====================================================================
	 */
	...
	/*
	 * Adds submenu pages to the MakerFaire Custom Post Type @access public =====================================================================
	 */
	public function add_menus() {
		add_submenu_page ( 'edit.php?post_type=mf_form', 'Project Images', 'Project Images', 'edit_others_posts', 'mf_project_images', array (
				&$this,
				'show_project_images' 
		) );
		add_submenu_page ( 'edit.php?post_type=mf_form', 'Reports', 'Reports', 'edit_others_posts', 'mf_reports', array (
				&$this,
				'show_reports_page' 
		) );
	}
	...
	/*
	 * Inits all included meta boxes for the MakerFaire Custom Post Type @access public =====================================================================
	 */
	public function add_meta_boxes() {
		global $post;
		
		if (isset ( $post->post_status ) && ($post->post_status != 'auto-draft')) {
			add_meta_box ( 'mf_summary', 'Summary', array (
					&$this,
					'meta_box' 
			), 'mf_form', 'normal', 'default' );
			add_meta_box ( 'mf_details', 'Details', array (
					&$this,
					'meta_box' 
			), 'mf_form', 'normal', 'default' );
			add_meta_box ( 'mf_logistics', 'Edit Form', array (
					&$this,
					'meta_box' 
			), 'mf_form', 'normal', 'default' );
			if (has_term ( 'presenter', 'type' )) {
				add_meta_box ( 'mf_presenter_promos', 'Eventbrite Promo Codes', array (
						&$this,
						'meta_box' 
				), 'mf_form', 'side', 'default' );
			}
			add_meta_box ( 'mf_presenter_promos', 'Eventbrite Promo Codes', array (
					&$this,
					'meta_box' 
			), 'event-items', 'side', 'default' );
		} else {
			add_meta_box ( 'mf_form_type', 'Application Type', array (
					&$this,
					'meta_box' 
			), 'mf_form', 'normal', 'default' );
			add_meta_box ( 'mf_exhibit', 'Exhibit Details', array (
					&$this,
					'meta_box' 
			), 'mf_form', 'normal', 'default', array (
					'type' => 'exhibit' 
			) );
			add_meta_box ( 'mf_performer', 'Performer Details', array (
					&$this,
					'meta_box' 
			), 'mf_form', 'normal', 'default', array (
					'type' => 'performer' 
			) );
			add_meta_box ( 'mf_presenter', 'Presenter Details', array (
					&$this,
					'meta_box' 
			), 'mf_form', 'normal', 'default', array (
					'type' => 'presenter' 
		...
		add_meta_box ( 'mf_save', 'Edit Application', array (
				&$this,
				'meta_box' 
		), 'mf_form', 'side', 'default' );
		add_meta_box ( 'mf_logs', 'Status Changes &amp; Notifications Sent', array (
				&$this,
				'meta_box' 
		), 'mf_form', 'normal', 'default' );
		
		if (isset ( $post->post_status ) && ($post->post_status != 'auto-draft')) {
			add_meta_box ( 'mf_maker_contact', 'Contact Info', array (
					&$this,
					'meta_box' 
			), 'mf_form', 'side', 'default' );
			add_meta_box ( 'mf_maker_info', 'Makers', array (
					&$this,
					'meta_box' 
			), 'mf_form', 'side', 'default' );
		}
	...
	/*
	 * Controller for all backend Meta Boxes @access public @param object $post Current Post to be edited @param array $args Include arguments for the meta box. =====================================================================
	 */
...
<div class="submitbox" id="submitpost">
		...
		name="id" type="hidden"
		value="<?php echo $post->post_status != 'auto-draft' ? intval( $post->ID ) : 0; ?>" />
		...
		<div class="misc-pub-section">
			...
			if (is_object ( $edit_flow ) && is_a ( $edit_flow, 'edit_flow' )) :
				foreach ( $edit_flow->custom_status->get_custom_statuses () as $cso ) {
					?>
											<option value="<?php echo esc_attr( $cso->slug ); ?>"
					title="<?php echo esc_attr( $cso->description ); ?>"
					<?php selected( $cso->slug, $post->post_status ); ?>>
	...
	<div id="major-publishing-actions">
		...
		<div id="publishing-action">
			<a href="<?php echo post_permalink( $post->ID ) ?>"
				...
				title="<?php esc_attr( the_title() ); ?>">View Post</a><br> <input
	...
	<p class="misc-pub-section num-revisions">
			...
			href="<?php echo esc_url( get_edit_post_link( absint( $kid[0]->ID ) ) ); ?>"><?php _ex( 'Browse', 'revisions' ); ?></a>
					<?php endif; ?>
					</p>
...
<input name="form_type" type="hidden"
	value="<?php echo esc_attr( $data->form_type ); ?>" />
<table style="width: 100%">
	<tr>
		<td style="width: 210px;" valign="top"><img
			src="<?php //echo esc_url( get_resized_remote_image_url( $photo, 200, 200 ) ); ?>"
		...
		<td valign="top">
			...
			?>
					...
					<td valign="top"><a
						href="<?php echo esc_url( $data->{ $wkey } ); ?>" target="_blank"><?php echo esc_url( $data->{ $wkey } ); ?></a></td>
				</tr>
				<tr>
			...
			?>
					...
					<td valign="top"><a
						href="<?php echo esc_url( $data->supporting_documents ); ?>"
						target="_blank"><?php echo esc_url( $data->supporting_documents ); ?></a></td>
				</tr>
				<tr>
					<td style="width: 80px;" valign="top"><strong>Layout:</strong></td>
					<td valign="top"><a href="<?php echo esc_url( $data->layout ); ?>"
						target="_blank"><?php echo esc_url( $data->layout ); ?></a></td>
				</tr>
							<?php endif; ?>
							<?php
...
// Loop through theme
				while ( $get_events->have_posts () ) :
					...
					<td valign="top"><a
						href="<?php echo admin_url(); ?>post-new.php?post_type=event-items&amp;refer_id=<?php echo absint( $parent_post_id ); ?>">Schedule
							Another Event</a></td>
				</tr>
								<?php } else { ?>
									<tr>
					...
					<td valign="top"><a
						href="<?php echo admin_url(); ?>post-new.php?post_type=event-items&amp;refer_id=<?php echo get_the_ID(); ?>">Schedule
							This Event</a></td>
				</tr>
								<?php
			...
			?>
					...
					<td valign="top"><input type="text" id="video-coverage"
						...
						value="<?php echo !empty( $event_record['mfei_coverage'][0] ) ? esc_url( $event_record['mfei_coverage'][0] ) : ''; ?>" />
						<input type="hidden" name="event-id"
						value="<?php echo get_the_ID(); ?>" /></td>
				</tr>
				<tr>
...
<?php echo esc_html( $data->private_city.', '.$data->private_state.' '.$data->private_zip.' '.$data->private_country ); ?><br />
<br />
<strong>Bio</strong>
<br />
...
<?php
			...
			if ($args ['id'] != 'mf_logistics') {
				$data = array (
						'form_type' => $args ['args'] ['type'],
						'maker_faire' => $this->maker_faire 
				);
...
<table style="width: 100%">
				...
				<?php foreach( (array) $data as $k => $v ) : if ( in_array( $k, $cont ) ) continue; ?>
					<tr class="mf-form-row" id="<?php echo esc_attr( $k ); ?>"
		<?php if ( strpos( $k, '_thumb' ) !== false ): ?>
...
<script type="text/javascript">
	 ...
	 * 
	 * @param Object $post
	 *        	The post object. Probably don't need it, but for now we'll pass it in
	 * @param Array $args
	 *        	An array arguments for meta information of the application
	 * @param Array $data
	 *        	All the json data saved for application
	 * @return HTML
	 *
	 * @since Mechani-Kong
	 */
	private function app_details($post, $args, $data) {
		...
		?>
<h3
	style="background: #ddd; color: #000; margin: 0; position: relative; top: 2px;">Application
	Information</h3>
...
$i = 0;
		foreach ( $data as $key => $value ) :
			...
			<td class="field-name" valign="top"
				style="width: 15%; font-weight: bold;"><?php echo esc_html( str_replace( '_', ' ', strtoupper( $key ) ) ); ?></td>
...
<h3
	style="background: #ddd; color: #000; margin: 30px 0 0; position: relative; top: 2px;">Maker
	Information</h3>
...
<?php
	...
	private function convert_content($key, $value) {
		if ($key == 'project_website' || $key == 'project_video' || $key == 'layout' || $key == 'group_website' || $key == 'presentation_website' || $key == 'video') {
			...
			// $output = '<a href="' . esc_url( $value ) . '"><img src="' . get_resized_remote_image_url( esc_url( $value ), 130, 130, true ) . '" width="130" height="130" target="_blank"></a>';
	...
	private function presenter_promos($post) {
		...
		?>
<input type="text" placeholder="Insert the unique Eventbrite promo code"
	name="presenter-promo-code"
	value="<?php echo ( ! empty( $promo ) ) ? esc_attr( $promo ) : ''; ?>"
	style="width: 100%;">
<?php
	...
	/*
	 * Creates Backend MakerFaire Application Forms @access public @param string $type Form type @param string $key Key for this field @param string $value Value of this field @param array $all_data All the data from the form =====================================================================
	 */
...
<div>
	<input name="<?php echo esc_attr( $type.'['.$key.']' ); ?>[]"
		type="checkbox" value="<?php echo esc_attr( $dkey ); ?>"
		<?php checked( in_array( $dkey, (array) $value ) ); ?> />
...
<div>
	<input name="<?php echo esc_attr( $type . '[' . $key . ']' ); ?>"
		type="radio" value="<?php echo esc_attr( $dkey ); ?>"
		<?php checked( $dkey == $value ); ?> />
					<?php echo esc_html( $data ); ?>
				</div>
<?php endforeach; ?>
...
<input name="<?php echo esc_attr( $type . '[' . $key . '][0]' ); ?>"
	value="<?php echo esc_attr( isset( $all_data[ $key ][0] ) ? $all_data[ $key ][0] : '' ); ?>"
	type="text" />
</td>
</tr>
<?php
...
<tr class="mf-form-row <?php echo esc_attr( $key ); ?>"
	id="<?php echo esc_attr( $fn ); ?>">
	<td valign="top"><?php echo esc_html( ucwords( str_replace( '_', ' ', $fn ) ) ); ?>:</td>

					<?php if ( $fn == 'm_maker_bio' || $fn == 'presenter_bio' ) :?>
						<td><textarea
			name="<?php echo esc_attr( $type . '[' . $fn . '][0]' ); ?>"
			id="<?php echo esc_attr( $type . '[' . $fn . '][0]' ); ?>" cols="30"
			rows="10"><?php echo ( ! empty( $data ) ) ? esc_attr( $data ) : ''; ?></textarea></td>
					<?php else : ?>
						<td><input style="width: 100%;"
		name="<?php echo esc_attr( $type . '[' . $fn . '][0]' ); ?>"
		value="<?php echo esc_attr( $data ); ?>" type="text" /></td>
					<?php endif; ?>
				</tr>
<?php
...
<tr class="mf-form-row add-maker">
	<td valign="top"><?php echo esc_html( $ftitle ); ?>:</td>
							...
							<?php if ( $fkey == 'm_maker_bio' || $fkey == 'presenter_bio' ) : ?>
								<td><textarea
			name="<?php echo esc_attr( $type . '[' . $fkey . '][' . $i . ']' ); ?>"
			id="<?php echo esc_attr( $type . '[' . $fkey . '][' . $i . ']' ); ?>"
			cols="30" rows="10"><?php echo esc_attr( $data ); ?></textarea></td>
							<?php else : ?>
								<td><input
		name="<?php echo esc_attr( $type . '[' . $fkey . '][' . $i . ']' ); ?>"
		value="<?php echo esc_attr( $data ); ?>" type="text" /></td>
...
<tr class="remove-maker">
	<td colspan="2"><input onclick="mf_remove_maker(this)" type="button"
		value="Remove Maker Above" class="button button-primary button-large">
	</td>
</tr>
<?php
		...
		<?php else : ?>
<input name="<?php echo esc_attr( $type . '[' . $key . ']' ); ?>"
	value="<?php echo $san_value; ?>" type="text" />
	...
	/*
	 * Updates mf_form object content and post meta data @access public @param integer $id Post id =====================================================================
	 */
	...
	/*
	 * Controller to deal with post status change @access public =====================================================================
	 */
	public function handle_post_status_change() {
		...
		$r = wp_mail ( $tos, $subject, $message, array (
	...
	/*
	 * MakerFaire Shortcode handler "mf_form" @access public @param array $atts An array of atributes included in the shortcode @return string The replacement text =====================================================================
	 */
	...
	/*
	 * Get's all forms and echos back for AJAX Listener @access public =====================================================================
	 */
	...
	/*
	 * Handles frontend form submission @access public =====================================================================
	 */
	public function ajax_handler() {
		...
		if (! in_array ( $res ['form_type'], array (
				'exhibit',
				'performer',
				'presenter' 
		) ))
			...
			foreach ( $files [$_POST ['form']] ['s' . $_POST ['step']] as $n ) {
				if (empty ( $errors ) && ((isset ( $_POST [$n] ) && $_POST [$n] != '') || isset ( $_FILES [$n] ))) {
					if (isset ( $_FILES [$n] ) && $_FILES [$n] ['name'] != '') {
						if ($r = $this->upload ( $_FILES [$n], ! in_array ( $n, array (
								'layout',
								'supporting_documents' 
						) ) )) {
							$file_res [$n] = $r ['url'];
	...
	/*
	 * Upload images / documents and save as WP attachments @access public @param array $ufile File object $t Form type @param boolean $require_size Check if deminsions are valid @return boolean|array False if error and upload array if valid =====================================================================
	 */
	...
	/*
	 * Create a MakerFAIRE Form @access public @param array $r Form data @param string $t Form type @return integer Form that's created's ID =====================================================================
	 */
	private function create_mf_form($r, $t) {
		...
		$d = array (
				'post_author' => 0,
				'post_content' => json_encode ( $r, JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE ),
				'post_title' => $n,
	...
	/*
	 * Update a MakerFAIRE Form @access public @param string $t Form type @param integer $id Form ID @param array $r Form data @param integer $s From Step @return integer Form that's updated ID =====================================================================
	 */
	private function update_mf_form($t, $id, $r, $s) {
		...
		/*
		 * if(isset($_POST['exhibit']) && isset($_POST['exhibit']['tags']) ) { $tag_string = $_POST['exhibit']['tags']; $tags_clean = array(); $tags = explode( ',', $tag_string ); foreach ($tags as $tag ) { $tags_clean[] = sanitize_title( $tag ); } wp_set_object_terms( $id, $tags_clean, 'post_tag'); }
		 */
		
		// Only used for wp-admin disabled 8.6.2014
		// need to break them out for categories...
		/*
		 * if( (isset($_POST['exhibit'])) && isset($_POST['exhibit']['cats']) ) { $cat_string = $_POST['exhibit']['cats']; $cats = explode( ',', $cat_string ); foreach ($cats as $cat ) { #wp_set_object_terms( $id, sanitize_title( $cat ), 'category', true ); } }
		 */
		...
		if ((isset ( $r ['tags'] )) && (is_array ( $r ['tags'] ))) {
			...
			foreach ( $tags as $tag ) {
				...
				$term = get_term_by ( 'slug', $tag_cleaned, 'post_tag' );
				if ($term !== false) {
	...
	/*
	 * Sends a email to form submitter @access public @param array $r Form data @param string $n Name @return boolean if wp_mail was successful. =====================================================================
	 */
	...
	/*
	 * Sends a email to addtional makers @access public @param integer $id Form ID @param array $emails Emails to send @param array $r Form data @param string $i Include body @param string $n Name @return boolean if wp_mail was successful. =====================================================================
	 */
	private function send_maker_invite_email($id, $emails, $r, $n) {
		...
		$r = wp_mail ( $emails, 'Maker Faire Application: ' . intval ( $id ) . ': ' . $app_name, $m, array (
				'Content-Type: text/html',
				'From: Maker Faire <makers@makerfaire.com>',
				'Bcc: Maker Faire <makers@makerfaire.com>' 
		) );
	...
	/*
	 * Tests if a phone number is valid @access public @param string $phone Phone to check @return boolean if valid phone number =====================================================================
	 */
	...
	/*
	 * Tests if a zipcode is valid @access public @param string $zip Zipcode to check @return boolean if valid zipcode =====================================================================
	 */
	private function is_zip($zip) {
		return ( bool ) preg_match ( '/^[0-9]{5}(-[0-9]{4}){0,1}$/', $zip );
	}
	/*
	 * Outputs a text field @access public @param string $n textbox name @param array $attr An array of attributes to include =====================================================================
	 */
	public function text($n, $attr = array()) {
		?>
<input type="text" name="<?php echo esc_attr( $n ); ?>"
	value="<?php echo esc_attr( $this->form[$n] ); ?>"
	<?php
...
<?php
	...
	/*
	 * Outputs a radio button @access public @param string $n radio name @param array $set An array of values and labels =====================================================================
	 */
	public function radio($n, $set) {
		foreach ( $set as $k => $v ) :
			...
			?>
<div>
	<input name="<?php echo esc_attr( $n ); ?>" type="radio"
		value="<?php echo esc_attr( $k ); ?>"
		<?php checked( $k, $this->form[$n] ); ?> /> <?php echo esc_html( $v ); ?></div>
<?php
		endforeach
		;
	}
	/*
	 * Outputs a checkbox @access public @param string $n checkbox name @param array $set An array of values and labels =====================================================================
	 */
	public function checkbox($n, $set) {
		foreach ( $set as $k => $v ) :
			...
			?>
<div>
	<input name="<?php echo esc_attr( $n ); ?>[]" type="checkbox"
		value="<?php echo esc_attr( $k ); ?>"
		<?php checked(in_array( $v, (array )$this->form[$n] ) ); ?> /> <?php echo esc_html( $v ) ?></div>
<?php
		endforeach
		;
	}
	/*
	 * Outputs a textarea @access public @param string $n textarea name @param array $attr An array of attributes to include =====================================================================
	 */
	public function textarea($n, $attr = array()) {
		?>
<textarea name="<?php echo esc_attr( $n ); ?>"
	<?php
		foreach ( $attr as $a => $av ) {
			echo esc_html ( $a ) . '="' . esc_attr ( $av ) . '" ';
		}
...
<?php
	...
	/*
	 * Outputs a select field @access public @param string $n Select name @param array $set An array of values and labels =====================================================================
	 */
...
<select name="<?php echo esc_attr( $n ); ?>">
	<option value="">-- Select One</option>
		...
		<?php selected( $v, $this->form[$n] ); ?>> <?php echo esc_html( $lbl ); ?></option>
			<?php endforeach; ?>
		</select>
<?php
	}
	/*
	 * Outputs a file field @access public @param string $n key of file path @param string $rn real key of path =====================================================================
	 */
	public function file($n, $rn) {
		if ($this->form [$n] == '') {
			...
			if ($rn == 'supporting_documents' || $rn == 'layout') {
				...
				?><img
	src="<?php echo esc_attr( $this->form[substr( $n, 0, -1 ).'_thumb]'] ); ?>"
	style="max-width: 600px" /><?php
			}
			
			?>
<input type="hidden" name="<?php echo esc_attr( $n ); ?>"
	value="<?php echo esc_attr( $v ); ?>" />
<input type="hidden"
	name="<?php echo esc_attr( substr( $n, 0, -1 ).'_thumb]' ); ?>"
	value="<?php echo esc_attr( $thumb ); ?>" />
<div id="<?php echo esc_attr( $rn ); ?>" class="info overwrite">Overwrite
	File</div>
<?php
		}
	}
	...
	/*
	 * Queue up all Admin JS and CSS @access public =====================================================================
	 */
	public function admin_enqueue($hook) {
		...
		if (('edit.php' == $hook && $post_type == 'mf_form') || ('post.php' == $hook && $post_type == 'mf_form'))
			wp_enqueue_script ( 'mf-custom-search', get_stylesheet_directory_uri () . '/plugins/maker-faire-forms/assets/js/search-id.js', array (
					'jquery' 
			), '1.0', true );
		
		wp_enqueue_script ( 'mf-reports', get_stylesheet_directory_uri () . '/plugins/maker-faire-forms/assets/js/reports.js', array (
				'jquery' 
		), '1.0', true );
	...
	/*
	 * Output list of project images @access public =====================================================================
	 */
...
<div class="wrap" id="iscic">
			...
			<h2><?php echo get_term_by( 'slug', MF_CURRENT_FAIRE, 'faire')->name; ?>
	...
	<h1><?php echo esc_html( ucwords( $type ) ); ?> Applications</h1>
			...
			foreach ( $list as $id => $form ) :
				?>
					<div
		style="float: left; width: 400px; margin: 20px; height: 500px;">
		<a href="<?php echo esc_url( get_edit_post_link( $id ) ) ?>"> <img
			src="<?php echo esc_url( $form['form_photo_thumb'] ); ?>"
			style="max-width: 400px; max-height: 400px; margin-bottom: 10px;" />
		</a>
...
<?php
	...
	/*
	 * Output Reports List @access public =====================================================================
	 */
	public function show_reports_page() {
		...
		if ('POST' == $_SERVER ['REQUEST_METHOD']) {
			...
			} elseif (isset ( $_POST ['mf_syncevents'] ) && wp_verify_nonce ( $_POST ['mf_syncevents'], 'mf_syncevents' )) {
				$offset = (isset ( $_POST ['offset'] )) ? intval ( $_POST ['offset'] ) : 0;
				$this->sync_events ( 0, $offset );
...
<div class="wrap" id="iscic">
	...
	<div style="width: 45%; float: left">
		...
		Syncs 100 applications at a time.<br />To do a full sync start at 0
		and increase by 100 until you're done.
		<form action="" method="post">
			<p>
			
			
			<div style="float: left; width: 50px">
				<strong>Start</strong><br /> <select name="offset">
			...
			<div class="clear"></div>
			<p class="submit">
				<input type="submit" value="Sync Statuses With JDB Now"
		...
		<form action="" method="post">
			<p class="submit">
				<input type="submit" value="Sync With JDB Now"
					class="button button-primary button-large" />
			</p>
		...
		<ul>
			<?php foreach( $fails as $fail ) : ?>
				<li><a
				href="post.php?post=<?php echo intval( $fail->ID ); ?>&amp;action=edit"><?php echo esc_html( $fail->post_title ); ?></a></li>
			<?php endforeach; ?>
			</ul>
	</div>
	<div
		style="width: 45%; float: right; border: 1px solid #DFDFDF; border-radius: 3px; padding: 10px; background: #F2F2F2">
		<h1>Generate Reports</h1>

		<form method="get" class="reports-form"
			style="border-bottom: 1px solid #dfdfdf">
			<input type="hidden" name="post_type"
				value="<?php echo esc_attr( $_REQUEST['post_type'] ); ?>" /> <input
				type="hidden" name="page"
				value="<?php echo esc_attr( $_REQUEST['page'] ); ?>" /> <input
				type="hidden" name="form_export" value="true" />
					<?php wp_nonce_field( 'mf_export_check', 'export_nonce', false ); ?>

					<ul>
						<?php if ( ! empty( $faires ) ) : ?>
							<li><label for="faire">Faire Applications</label> <select
					name="faire" id="faire">
									<?php foreach ( $faires as $faire ) : ?>
										<option value="<?php echo esc_attr( $faire->slug ); ?>"
							<?php selected( $faire->slug, MF_CURRENT_FAIRE ); ?>><?php echo esc_html( $faire->name ); ?></option>
									<?php endforeach; ?>
								</select></li>
						<?php endif; ?>
						<li><label for="app-type">Application Type</label> <select
					name="type" id="app-type">
				...
				<li><label for="post-status">Application Status</label> <select
					name="post_status" id="post-status">
...
foreach ( $wp_post_statuses as $status => $name ) {
		...
		?>
				...
				<li><label for="categories">Category</label>
							<?php
...
wp_dropdown_categories ( array (
		...
		?>
				...
				<li><label for="tags">Tags</label> <select name="tag" id="tags">
						<option value="all">All Tags</option>
								<?php foreach( $tags as $tag ) : ?>
									<option value="<?php echo esc_attr( $tag->slug ); ?>"><?php echo esc_html( $tag->name ); ?></option>
								<?php endforeach; ?>
							</select></li>
				<li><label for="location">Location</label> <select name="location"
					id="location">
				...
				<li><label for="makers">Makers</label> <input type="checkbox"
					name="makers" id="makers" value="true" /> Export Maker Based Report
				</li>
				<li style="margin: 20px 0;"><input type="submit"
					class="button button-primary button-large" value="Process Report" />
				</li>
			</ul>
		</form>
		...
		<h3>
			<a
				href="<?php echo wp_nonce_url( 'edit.php?post_type=mf_form&page=mf_reports&maker_csv=presenter', 'mf_export_check' ); ?>">Export
				Editorial Comments</a>
		</h3>

		<h2 style="margin-top: 40px;">Exhibit Reports</h2>
		<h3>
			<a
				href="<?php echo wp_nonce_url( 'edit.php?post_type=mf_form&page=mf_reports&exhibit_signage_csv', 'mf_export_check' ); ?>">Export
				Exhibit Signage</a>
		</h3>
		<h3>
			<a
				href="<?php echo wp_nonce_url( 'edit.php?post_type=mf_form&page=mf_reports&exhibit_signage_csv&reprint', 'mf_export_check' ); ?>">Export
				Reprints</a>
		</h3>
		<h3>
			<a
				href="<?php echo wp_nonce_url( 'edit.php?post_type=mf_form&page=mf_reports&exhibit_signage_clear_reprint', 'mf_export_check' ); ?>">Clear
				Reprints</a>
		</h3>
		...
		<h3>
			<a
				href="<?php echo wp_nonce_url( 'edit.php?post_type=mf_form&page=mf_reports&presentation_csv=manager', 'mf_export_check' ); ?>">Stage
				Manager Report</a>
		</h3>
		<h3>
			<a
				href="<?php echo wp_nonce_url( 'edit.php?post_type=mf_form&page=mf_reports&presentation_csv=signage', 'mf_export_check' ); ?>">Stage
				Signage Report</a>
		</h3>
		<h3>
			<a
				href="<?php echo wp_nonce_url( 'edit.php?post_type=mf_form&page=mf_reports&presentation_csv=checkin', 'mf_export_check' ); ?>">Presenter
				CheckIn Report</a>
		</h3>
		  <?php
		...
		?>
			...
			Syncronizes tags/cats on 100 applications at a time.<br />To do a
			full sync start at 0 and increase by 100 until you're done.
			<p>
			...
			<div class="clear"></div>
			<p class="submit">
				<input type="submit" value="Sync Tags"
					class="button button-primary button-large" />
			</p>
				<?php wp_nonce_field( 'mf_synctags', 'mf_synctags' ); ?>
				<?php echo("<p>Last run(".esc_html(get_option('mf_sync_tags_offset'))."): ".esc_html(date("d M Y h:i a",get_option('mf_sync_tags_runtime')))."</p>"); ?>
			</form>

		<form action="" method="post" style="border-top: 1px solid #dfdfdf;">
			<h2 style="margin-top: 20px;">Sync Applications to Makers</h2>
			Syncronizes maker_id/maker_email on 100 applications at a time.<br />To
			do a full sync start at 0 and increase by 100 until you're done.
			<p>
			...
			<div class="clear"></div>
			<p class="submit">
				<input type="submit" value="Sync Makers"
					class="button button-primary button-large" />
			</p>
		...
		?>
			...
			<div class="clear"></div>
			<p class="submit">
				<input type="submit" value="Sync Events"
					class="button button-primary button-large" />
			</p>
...
<?php
	...
	/*
	 * Gathers and outputs application data @access private @return array Associative array of all reports stats =====================================================================
	 */
	 ...
	 *
	 * @param array $options
	 *        	The array of filters we want to pass to WP_Query like application type, app status, and other fun stuff.
	 * @param boolean $return_array
	 *        	Returns an array instead of the CSV
	 * @return void
	 */
	...
	/*
	 * Associate all posts with EditFlow Data @access private @param array $efterms An array of all EditFlow Terms/Types @return array An Array of all posts associated with EditFlow Data =====================================================================
	 */
	...
	/*
	 * Gets all EDITFLOW Terms and Types @access private @return array An Array of all edit flow terms/types =====================================================================
	 */
	private function get_editflow_terms() {
		$efterms = array ();
		$terms = get_terms ( 'ef_editorial_meta', array (
				'hide_empty' => false 
		) );
	...
	/**
	 * Exports all Makers in an awesome, fancy, CSV file that you can use to organize Maker Faire.
	 * #magic. (Unicorns not included).
	 * 
	 * @param array $options
	 *        	The array of filters we want to pass to WP_Query like application type, app status, and other fun stuff.
	 * @return void
	...
	/*
	 * Gathers and builds the output for EDITORIAL COMMENTS @access private =====================================================================
	 */
	...
	/*
	 * Gathers and builds the output for PRESENTER EXPORTS @access private =====================================================================
	 */
	...
	/*
	 * Combines certain attributes into single for output @access private @param string $key The key to look for @param boolean|string $reverse The form type to reverse look up or false =====================================================================
	 */
	...
	/*
	 * Sets CSV headers and outputs all data @access private @param string $name The name of the ouput CSV @param string $data The data to be in the CSV =====================================================================
	 */
	...
	/*
	 * Get all posts with type mf_form @access public @param string|null $sort Whether and how the forms should be sorted. @param string $status The status of the application @return array Maker Faire Forms =====================================================================
	 */
	public function get_all_forms($sort = NULL, $app_status = 'all', $filters = array(), $faire = '') {
			...
			$args ['meta_query'] [] = array (
					array (
							...
							'value' => serialize ( array (
									absint ( $filters ['location'] ) 
							) )  // Sadly we are saving locations as a serialized array.... soooooo
										) 
			);
		}
		...
		foreach ( $posts as $post ) {
			...
			$res [$form [$sort]] [$post->ID] = array_merge ( $form, array (
	...
	/*
	 * Get all posts that have failed to sync with JDB @access private @return array WP Posts =====================================================================
	 */
	...
	/*
	 * Determines if a field is textarea @access private @param string $key The key to check @return boolean If the key is a textarea =====================================================================
	 */
	...
	/*
	 * Fix newlines saving improperly to at least display correctly. @access private @param string $str String to search @param string $replace The string to replace the bad characters with. =====================================================================
	 */
	...
	/*
	 * Sync all MakerFiare Application Objects with External JDB @access private @param int $id Post id to SYNC =====================================================================
	 */
	private function sync_jdb($id = 0) {
		self::gravityforms_sync_jdb ();
		/*
		 * // Setup a list of our local servers... $local_server = array( 'localhost', 'make.com', 'vip.dev', 'staging.makerfaire.com' ); // Don't sync from any of our testing locations. if ( isset( $_SERVER['HTTP_HOST'] ) && in_array( $_SERVER['HTTP_HOST'], $local_server ) ) return false; if ( ! $id ) { $posts = $this->get_all_forms(); } else { $post = get_post( $id ); if ( is_null( $post ) ) return false; if ( $post->post_type != 'mf_form' ) return false; $posts = array( $post ); } foreach ( $posts as $post ) { $form = (array) json_decode( str_replace( "\'", "'", $post->post_content ) ); $res = wp_remote_post( 'http://db.makerfaire.com/updateExhibitInfo', array( 'body' => array_merge( array( 'eid' => $post->ID, 'mid' => $form['uid'] ), (array) $form ) ) ); if ( 200 == wp_remote_retrieve_response_code( $res ) ) { $body = json_decode( $res['body'] ); if ( $body->exhibit_id == '' && $body->exhibit_id == 0 ) { update_post_meta( $post->ID, 'mf_jdb_sync_fail', time() ); } else { update_post_meta( $post->ID, 'mf_jdb_sync', time() ); delete_post_meta( $post->ID, 'mf_jdb_sync_fail' ); } } else { update_post_meta( $post->ID, 'mf_jdb_sync_fail', time() ); } } if ( ! $id ) update_option( 'mf_full_jdb_sync', date( 'M jS, Y g:s A', ( time() - ( 3600 * 7 ) ) ) );
		 */
	}
	/*
	 * Sync MakerFaire Application Statuses @access private @param int $id Post id to SYNC @param string $app_status Post status =====================================================================
	 */
	...
	/*
	 * Sync MakerFiare Application Statuses @access private @param int $offset Offset of posts to sync @param int $length Length of sync batch =====================================================================
	 */
	private function sync_all_status_jdb($offset = 0, $length = 100) {
		...
		$result = $mysqli->query ( "Select  a.id,b.value from wp_rg_lead a join wp_rg_lead_detail b on a.id=b.lead_id and b.field_number='303'
				where  a.status <> 'trash' and a.id in (50188, 50155, 50162, 50209, 50334) and a.form_id in (20,13,12,16,17) and a.id not in (select lead_id from wp_rg_lead_meta where meta_key='mf_jdb_status_sync') limit 100
		" );
	...
	/*
	 * Sync all MakerFaire Application Objects with Tags/Cats @access private @param int $id Post id to SYNC =====================================================================
	 */
				...
				foreach ( $tags as $tag ) {
					...
					if ($term !== false) {
						...
						if ($tag_split_count > 1) {
								...
								if ($term_split !== false) {
									...
									if ($term_split !== false) {
										...
										if ($term_split !== false) {
											...
											$term_split = get_term_by ( 'slug', $tag_3, 'post_tag' );
											if ($term_split !== false) {
							...
							$term_split = get_term_by ( 'slug', $tag_split [0], 'post_tag' );
						}
					}
				}
			} // End Tags
	...
	/*
	 * Sync all MakerFaire Application Objects with Makers @access private @param int $id Post id to SYNC =====================================================================
	 */
		...
		while ( $query->have_posts () ) :
			...
			if (isset ( $json_post->maker ) && property_exists ( $json_post, 'maker' ) && ($json_post->maker == 'A list of makers')) {
				if (isset ( $json_post->m_maker_name ) && property_exists ( $json_post, 'm_maker_name' ) && (count ( $json_post->m_maker_name ) > 1)) {
					$this->add_maker_from_app ( $json_post, $json_post->m_maker_name, get_the_ID () );
				...
				if (! empty ( $json_post->performer_name )) {
					$this->add_maker_from_app ( $json_post, array (
							$json_post->performer_name 
					), get_the_ID (), true, 'performer' );
				}
			}
		endwhile
		;
		
		update_option ( 'mf_sync_makers_runtime', time () );
		update_option ( 'mf_sync_makers_offset', $offset );
	}
	
	/*
	 * Sync all MakerFaire Events Objects with Makers @access private @param int $id Post id to SYNC =====================================================================
	 */
	private function sync_events($id = 0, $offset = 0) {
		...
		while ( $query->have_posts () ) :
			...
			if (isset ( $json_post->maker ) && property_exists ( $json_post, 'maker' ) && ($json_post->maker == 'A list of makers')) {
				...
				if (! empty ( $json_post->performer_name )) {
					$this->add_makers_to_event ( $json_post, array (
							$json_post->performer_name 
					), $event_id, true, 'performer' );
				}
			}
		endwhile
		;
	...
	/*
	 * Add Maker from Application @access private
	 */
	...
	/*
	 * Add Makers to event @access private
	 */
	...
	/*
	 * Upgrade plugin appropriately. @access private =====================================================================
	 */
	...
	/**
	 * Creates new makers in our Makers custom post type.
	 * Primarily used for our mobile app for the time being.
	 * 
	 * @param array $maker
	 *        	The array of a single maker
	 * @param string $faire_slug
	 *        	The default faire we want to associate this maker to.
	 */
	public function add_to_maker_cpt($maker, $faire_slug = '') {
		...
		$existing_maker_title = get_page_by_title ( htmlspecialchars ( $maker ['title'], ENT_QUOTES ), OBJECT, 'maker' );
	...
	public static function gravityforms_sync_jdb() {
		...
		$result = $mysqli->query ( "Select  id,form_id from wp_rg_lead where  status <> 'trash'  and form_id in (20,13,12,16,17) and id not in (select lead_id from wp_rg_lead_meta where meta_key='mf_jdb_sync') LIMIT 100" );
	...
	public static function gravityforms_to_jdb_record($lead, $lead_id, $form_id) {
		...
		foreach ( $fieldtopics ['inputs'] as $topic ) {
			if (strlen ( $lead [$topic ['id']] ) > 0)
				$topicsarray [] = $lead [$topic ['id']];
		}
		// Load Plans
		$fieldplans = RGFormsModel::get_field ( $form, '55' );
		$plansarray = array ();
		foreach ( $fieldplans ['inputs'] as $plan ) {
			if (strlen ( $lead [$plan ['id']] ) > 0)
				$plansarray [] = $lead [$plan ['id']];
		}
		// Load Loctations
		...
		foreach ( $fieldlocations ['inputs'] as $location ) {
			if (strlen ( $lead [$location ['id']] ) > 0)
				$locationsarray [] = $lead [$location ['id']];
		}
		// Load RF
		...
		foreach ( $rfinputs ['inputs'] as $rfinput ) {
			if (strlen ( $lead [$rfinput ['id']] ) > 0)
				$rfarray [] = $lead [$rfinput ['id']];
		}
		// Load statuses
